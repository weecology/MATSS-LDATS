...
acf(rowSums(portal_ann_data$abundance))

all_data_out <- subset_data(data = portal_ann_data, n_segs = 28, sequential = T, buffer = 2)

one_data_out <- subset_data(data = portal_ann_data, n_segs = 28, sequential = T, buffer = 2, which_seg = 1)
...
```{r set up LDAs}
one_lda <- run_LDA(data = one_data_out, max_topics = 3, seed = 2)
