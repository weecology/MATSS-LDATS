...
all_lik_names <- cached(cache = cache)[ which(grepl(cached(cache = cache), pattern = "full_lik"))]

full_lik_results <- lapply(all_lik_names, FUN = readd, character_only = T, cache = cache)

names(full_lik_results) <- all_lik_names
```
...
make_aic_plot <- function(model_data) {
    ggplot(data = model_data, aes(x = seed, y = TS_AICc, colour = seed)) + 
        ...
        theme_bw() +
        facet_grid(rows = vars(ts_model_desc), cols = vars(as.integer(k)), scales = "fixed", switch = "y") +
...
make_best_mean_aicc_plot <- function(model_data) {
    ggplot(data = model_data, aes(x = as.integer(k), y = mean_aicc, colour = ts_model_desc)) + 
...
make_best_median_aicc_plot <- function(model_data) {
    ggplot(data = model_data, aes(x = as.integer(k), y = median_aicc, colour = ts_model_desc)) + 
        geom_jitter(width = 0.05, height =0) +
        theme_bw() +
