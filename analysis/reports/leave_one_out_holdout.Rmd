...
load(file = here::here("analysis", "loo", "all_model_eval_holdout.Rds"))

year_assignments <- data.frame(
    timestep = 1:nrow(full_data$abundance),
    assignment = NA
)

for(i in 1:5){
    load(file = here::here("analysis", "loo", paste0("loo_ts_", i, ".Rds")))
    
    these_years <- setdiff(full_data$covariates$year, this_seg$lda_models$data$covariates$year)
    
    these_timesteps <- which(full_data$covariates$year %in% these_years)
    
    year_assignments$assignment[these_timesteps] <- i
}
...
avg_performance <- all_model_eval_h %>%
...
prediction_models <- all_model_eval_h %>%
...
all_pred <- bind_rows(all_pred, observed_with_ks)
    ...
    all_pred <- all_pred %>%
        left_join(year_assignments, by = "timestep") %>%
        mutate(in_segment = segment == assignment)


```

### Absolute abundances
```{r plot absolute abund predictions, fig.dim = c(10, 60)} 
...
### Relative abundances
```{r plot relative abundances, fig.dim = c(10, 60)}
rel_pred <- all_pred %>%
    group_by(timestep, source, k, segment, in_segment) %>%
...
```{r observed-predicted plots, fig.dim=c(9, 60)}

obs_abund <- rel_pred %>%
    filter(source == "observed") %>%
    select(timestep, species, abundance) %>%
    distinct() %>%
    rename(obs_abund = abundance)

obs_pred <- rel_pred %>%
    filter(source != "observed") %>%
    left_join(obs_abund, by = c("timestep", "species"))

obs_pred_plot <- ggplot(data = obs_pred, aes(x = obs_abund, y = abundance, color = as.factor(segment))) +
    geom_point(alpha = .5) +
    theme_bw() +
    facet_grid(rows = vars(species, in_segment), cols = vars(k), switch = "y") +
    geom_line(aes(x = obs_abund, y = obs_abund), color = "black") +
    theme(legend.position = "none") +
    xlim(min(obs_pred$abundance, obs_pred$obs_abund), max(obs_pred$abundance, obs_pred$obs_abund)) +
    ylim(min(obs_pred$abundance, obs_pred$obs_abund), max(obs_pred$abundance, obs_pred$obs_abund))
