...
load(file = here::here("analysis", "loo", "all_model_eval_sequential_holdout.Rds"))
...
avg_performance <- all_model_eval_s_h %>%
...
prediction_models <- all_model_eval_s_h %>%
...
all_pred <- bind_rows(all_pred, observed_with_ks)
 ...
 seg_starts <- floor(seq.int(0, nrow(full_data$abundance) +1, length.out = 6))
 
 year_assignments <- data.frame(
     timestep = unique(all_pred$timestep)
 )
 
 for(i in 1:length(year_assignments$timestep)) {
 year_assignments$assignment[i] <- min(which(seg_starts >= year_assignments$timestep[i])) - 1
 }
    all_pred <- all_pred %>%
...
### Absolute abundances
```{r plot absolute abund predictions, fig.dim = c(10, 20)} 
...
### Relative abundances
```{r plot relative abundances, fig.dim = c(10, 20)}
rel_pred <- all_pred %>%
    group_by(timestep, source, k, in_segment, segment) %>%
